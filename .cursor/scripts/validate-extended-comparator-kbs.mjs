#!/usr/bin/env node
/**
 * Validate Extended comparator evidence maps + workbooks against CN v0.3 and VCF KB.
 * Hyper-V must not share Azure Local's scoring signature.
 */
...
const PLATFORMS = [
  { key: 'Harvester', prefix: 'HRV', fileBase: 'Harvester' },
  { key: 'HyperV', prefix: 'HYP', fileBase: 'HyperV' },
  { key: 'CitrixHypervisor', prefix: 'CTX', fileBase: 'CitrixHypervisor' },
  { key: 'ScaleComputingHC3', prefix: 'SC3', fileBase: 'ScaleComputingHC3' },
  { key: 'XCPng', prefix: 'XCP', fileBase: 'XCPng' },
];

const CORE_BASES = ['AzureLocal', 'OpenShiftVirtualization', 'OpenStackKVM', 'ProxmoxVE'];
...
function main() {
  ...
  for (const base of CORE_BASES) {
    const p = join(KBS, `${base}_Evidence_Crosswalk_v1.json`);
    if (existsSync(p)) {
      const xw = load(p);
      coreSigs[base] = `${xw.counts['Full Parity']}/${xw.counts['Partial Parity']}/${xw.counts['No Parity']}`;
    }
  }
  ...
  for (const plat of list) {
    ...
    if (map.comparatorType !== 'Extended Comparator') {
      errors.push(`${label}: comparatorType must be Extended Comparator`);
    }
    ...
    if (map.evaluationBasisType === 'CompositePeriod') {
      errors.push(`${label}: CompositePeriod not expected for Extended comparators`);
    }
    ...
    for (const [coreName, coreSig] of Object.entries(coreSigs)) {
      if (sig === coreSig) errors.push(`${label}: scoring signature must differ from Core ${coreName} ${coreSig}`);
    }
    if (plat.key === 'HyperV' && coreSigs.AzureLocal && sig === coreSigs.AzureLocal) {
      errors.push(`${label}: Hyper-V must not match Azure Local ${coreSigs.AzureLocal}`);
    }
    if (seen[sig]) errors.push(`${label}: duplicate Extended signature vs ${seen[sig]}`);
    seen[sig] = label;
  ...
  console.log('\nPASS extended comparator KB validation');
}
